...
#条件を指定して研究者リスト作成する
def sk_execute(topic_ids,primary=True,threshold=15,year_threshold=2015,title_and_abstract_search='',max_works=16,di_calculation=False,output_sheet_name="API動作確認"):
    
    try:
        add_log("SpreadsheetManagerの初期化を開始します。")
        sheet_manager = SpreadsheetManager("アプリケーション", output_sheet_name)
        
        add_log("並列処理用のパラメータを設定中...")
        person_num = 4
        ...
        add_log("論文データの取得を開始します。")
        creater.run_get_works()
        add_log(f"取得した論文数: {len(creater.all_results)}")
       
        creater.extract_authors(only_japanese=True)
        add_log(f"抽出した日本人著者数: {len(creater.authors_id_list)}")
        ...
        while attempt < max_retries:
            try:
                ...
                attempt += 1
                add_log(f"エラーが発生しました (試行回数: {attempt}/{max_retries}): {e}")
               # print(f"エラーが発生しました (試行回数: {attempt}/{max_retries}): {e}")
                if attempt < max_retries:
                    print(f"再試行します...")
                else:
                    add_log(f"最大リトライ回数に達しました。操作を終了します。エラー:{e}")
