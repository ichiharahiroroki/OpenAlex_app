...
def create_author_profile(author_work_data_list: List[AuthorWorkData]) -> AuthorProfileData:
    ...
    # 最新の論文を取得
    latest_work = sorted_works[0]

    # 最新の所属機関と国コードを取得
    if latest_work.affiliation:
        profile.latest_affiliation = [latest_work.affiliation]
    if latest_work.country_codes:
        profile.latest_country = [latest_work.country_codes]
