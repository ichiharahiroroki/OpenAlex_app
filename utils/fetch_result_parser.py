    ...
    def works_dict_list_from_works_results(all_results):
        try:
            ...
            for work in all_results:
                ...
                # 新しいフィールドを作成し、それぞれの名前とIDを格納
                # article_dict["topics_name_id"] = [
                #     {"id": topic.get("id", "N/A"), "display_name": topic.get("display_name", "N/A")}
                #     for topic in work.get("topics", [])
                # ]

                # article_dict["subfield_name_id"] = [
                #     {
                #         "id": topic.get("subfield", {}).get("id", "N/A"),
                #         "display_name": topic.get("subfield", {}).get("display_name", "N/A")
                #     }
                #     for topic in work.get("topics", [])
                # ]

                # article_dict["domain_name_id"] = [
                #     {
                #         "id": topic.get("domain", {}).get("id", "N/A"),
                #         "display_name": topic.get("domain", {}).get("display_name", "N/A")
                #     }
                #     for topic in work.get("topics", [])
                # ]
...
if __name__ == "__main__":
    ...
    print(author_dict_list[4]["Keywords"])