...
class Calculater:
    ...
    def calculate_disruption_index_for_article(article,found_date=""):
        # フォーカルペーパーのID
        ...
        if not found_date:
            cited_by_api_url = article["cited_by_api_url"]+f",type_crossref:journal-article"
        else:
            cited_by_api_url = article["cited_by_api_url"]+f",type_crossref:journal-article,publication_date:<{found_date}"
    ...
    def count_citations_from_other_field(focal_article,result_list):
        ...
        """
        #print(focal_article)
        primary_topic_dict = focal_article.get("Primary Topic",{})
        focal_topic_id = primary_topic_dict["id"]
        focal_subfield_id = primary_topic_dict["subfield"]["id"]
        focal_field_id = primary_topic_dict["field"]["id"]
        focal_domain_id = primary_topic_dict["domain"]["id"]
        # カウント用の辞書を初期化
        citation_counts = {
            'topic': 0,
            'subfield': 0,
        ...
        for result in result_list:
            ...
            if not isinstance(result_primary_topic, dict):
                continue
            result_topic_id = result_primary_topic.get("id",None)
            if not result_topic_id:
                continue
            result_subfield_id = result_primary_topic.get("subfield", {}).get("id")
            result_field_id = result_primary_topic.get("field", {}).get("id")
            result_domain_id = result_primary_topic.get("domain", {}).get("id")
            
            # 各カテゴリごとに異なるかをチェックし、カウントを増加
            if result_topic_id and focal_topic_id and result_topic_id != focal_topic_id:
                citation_counts['topic'] += 1
            if result_subfield_id and focal_subfield_id and result_subfield_id != focal_subfield_id:
                citation_counts['subfield'] += 1
            if result_field_id and focal_field_id and result_field_id != focal_field_id:
                citation_counts['field'] += 1
            if result_domain_id and focal_domain_id and result_domain_id != focal_domain_id:
